...
PROXY_BASE = 'https://r.jina.ai/http://maoaujc.com/macaujc2/'
...
def parse_source(html):
    """Parse the public 3-minute Macau draw page.
    Supports both the original HTML layout and plain/Markdown text returned by a reader proxy.
    """
    ...
    out = []
    issue_re = re.compile(r'第(20\d{9})期')
    time_re = re.compile(r'(20\d{2})[-/](\d{1,2})[-/](\d{1,2})\s+(\d{1,2}):(\d{2}):(\d{2})')
    combo_re = re.compile(r'(\d{1,2})\s*[\u4e00-\u9fff]{0,3}\s+(\d{1,2})\s*[\u4e00-\u9fff]{0,3}\s+(\d{1,2})\s*[\u4e00-\u9fff]{0,3}\s+(\d{1,2})\s*[\u4e00-\u9fff]{0,3}\s+(\d{1,2})\s*[\u4e00-\u9fff]{0,3}\s+(\d{1,2})\s*[\u4e00-\u9fff]{0,3}\s*\+\s*(\d{1,2})')

    def parse_window(window, inline=''):
        text = '\n'.join(window)
        if inline:
            text = inline + '\n' + text
        ...
        # Preferred: six zodiac-tagged numbers + '+' + special in one line.
        cm = combo_re.search(text)
        if cm:
            nums = [f'{int(cm.group(i)):02d}' for i in range(1, 7)]
            special = f'{int(cm.group(7)):02d}'
            return open_time, nums, special

        # Original page layout: six separate number/zodiac lines, then '+', then special.
        nums, special, saw_plus = [], None, False
        for x in window:
            if time_re.search(x):
                continue
    ...
    for i, line in enumerate(lines):
        ...
        window = lines[i + 1:i + 35]
        open_time, nums, special = parse_window(window, line)
        if open_time and len(nums) == 6 and special:
            out.append((issue, open_time, nums, special))

    # Deduplicate in case a proxy renders a row more than once.
    seen = set()
    unique = []
    for row in out:
...
def fetch_page(page):
    """Read one 3-minute public-history page.
    Prefer the public read-only reader because Render may refuse direct access
    to the source host. Direct access is only a quick fallback.
    """
    target = f'http://maoaujc.com/macaujc2/?id=3&page={page}'
    proxy_url = PROXY_BASE + '?id=3&page=' + str(page)
    proxy_error = None
    try:
        ...
        if draws:
            return draws
        proxy_error = '备用读取内容无法解析'
    except Exception as e:
        proxy_error = str(e)

    try:
        r = requests.get(BASE_URL, params={'id': '3', 'page': str(page)},
                         headers=HEADERS, timeout=8)
        r.raise_for_status()
        draws = parse_source(r.text)
        if draws:
            return draws
        raise RuntimeError('原站返回内容无法解析')
    except Exception as e:
        raise RuntimeError(f'数据页读取失败（第{page}页）：备用读取：{proxy_error}；原站：{e}')
...
def sync(full=False):
    today = datetime.now(TZ8).date()
    all_draws = []
    pages = range(1, 13) if full else range(1, 3)
...
def bg():
    ...
    while True:
        try:
            # 首次启动或跨日时最多检查前12页，直到翻到今天以前；平时只检查前两页。
