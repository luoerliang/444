...
def save(issue,nums,source='telegram',force=False):
    if len(nums)!=7 or len(set(nums))<7 or not all(1<=n<=49 for n in nums):
        return False
    if not re.fullmatch(r'20\d{9}', str(issue)):
        return False
    pri={'legacy':0,'web':1,'api':2,'telegram':3}
    if source not in pri:
        return False
    c=sqlite3.connect(DB_PATH)
    try:
        try: c.execute("ALTER TABLE draws ADD COLUMN source TEXT DEFAULT 'legacy'")
        ...
        old=row[0] if row else 'legacy'
        # 机器人永远最高优先级；官网/API只允许补空缺，不能覆盖机器人原始号码。
        if row and pri.get(old,0) > pri.get(source,0):
            return False
        if row and old=='telegram' and source=='telegram' and not force:
            return False
    ...
    while True:
        try:
            url=(f'https://api.telegram.org/bot{BOT_TOKEN}/getUpdates'
                 f'?timeout=25&offset={offset}&allowed_updates=%5B%22message%22,%22channel_post%22,%22edited_message%22,%22edited_channel_post%22%5D')
            req=urllib.request.Request(url,headers={'User-Agent':'macau3/1.0','Accept':'application/json'})
            with urllib.request.urlopen(req,timeout=35) as r:
                d=json.loads(r.read().decode('utf-8','ignore'))
            for x in d.get('result',[]):
                offset=max(offset,int(x.get('update_id',0))+1)
                for k in ('message','channel_post','edited_message','edited_channel_post'):
                    msg=x.get(k) or {}
                    p=parse(msg.get('text','') or msg.get('caption',''))
                    if p:
                        save(p[0],p[1],source='telegram')
                        break
        except Exception:
            time.sleep(3)
...
@app.get('/health')
def health(): ds=draws(); return jsonify({'ok':True,'service':'macau3','telegram_configured':bool(BOT_TOKEN),'total':len(ds),'latest':(ds[0]['issue'] if ds else None),'today':sum(1 for d in ds if d['issue'].startswith(datetime.now(BJ).strftime('%Y%m%d')))})
...
# 启动顺序固定：建库 → 导入机器人历史 → 启动 Telegram 实时接收。
# 这样即使 Telegram 暂时没有新消息，网页也不会是空白。
init()
seed_robot_history()
threading.Thread(target=tg,daemon=True,name='telegram-receiver').start()
